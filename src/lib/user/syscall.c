...
/* Added for project 3: buffer cache custom tests*/
void cache_reset(void) { syscall0(SYS_CACHE_RESET); }

int get_cache_hit(void) { return syscall0(SYS_GET_CACHE_HIT); }

int get_cache_miss(void) { return syscall0(SYS_GET_CACHE_MISS); }

int get_blocks_read(void) { return syscall0(SYS_BLOCKS_READ); }

int get_blocks_write(void) { return syscall0(SYS_BLOCKS_WRITE); }
